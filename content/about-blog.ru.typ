...
Но с таким подходом не работает выделение текста, потому что пока что Typst растеризует текст (#link("https://github.com/typst/typst/issues/1239")[есть запрос], чтобы текст вставлялся как есть), когда экспортирует в SVG, а ```typ #html.frame()``` как раз рендерит контент в SVG
